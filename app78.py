...
# ---------------------------
# Namma Metro Asset Data (including Phase 2 corridors)
# ---------------------------
# Existing lines
...
# Phase 2 – new corridors (Reach 6, 2A, 2B – Airport line)
phase2_stations = [
    "Bannerghatta Road", "Hulimavu", "Ibbaluru", "Vellara Junction", "MICO Layout",
    "Konanakunte Cross", "Kothanur", "Jarganahalli", "Hosakerehalli", "Nagarabhavi",
    "Vijayanagar", "Attiguppe", "Deepanjali Nagar", "Mysore Road", "Nayandahalli",
    "Rajajinagar", "Mahalakshmi", "Sandal Soap Factory", "Yeshwanthpur", "Goraguntepalya",
    "Peenya", "Peenya Industry", "Jalahalli", "Dasarahalli", "Nagasandra",
    "Silk Board", "HSR Layout", "Agara", "Bellandur", "Marathahalli", "KR Puram",
    "Hoodi", "Garudacharapalya", "Singayyanapalya", "Doddanekkundi", "Mahadevapura",
    "Kumbena Agrahara", "Channasandra", "Whitefield", "ITPL", "Kadugodi"
]

depots = ["Peenya Depot", "Baiyappanahalli Depot", "Kengeri Depot", "Phase 2 Depot (KR Puram)"]
control_centers = ["Central Control Room - Byappanahalli", "Backup Control Center - Peenya"]

all_assets = purple_line_stations + green_line_stations + phase2_stations + depots + control_centers
...
def get_line(asset):
    if asset in purple_line_stations:
        ...
        return "Green Line"
    elif asset in phase2_stations:
        return "Phase 2 (Airport / Outer Ring Road)"
...
def get_zone(asset):
    ...
    if "Baiyappanahalli" in asset or "Indiranagar" in asset or "Trinity" in asset:
        ...
        return "West"
    elif "Silk Board" in asset or "KR Puram" in asset or "Whitefield" in asset:
        return "Outer Ring Road Corridor"
    else:
        return "Other"

# Coordinates (only a subset for demo; others default)
station_coords = {
    ...
    # Phase 2 stations – approximate coordinates
    "Silk Board": (12.9141, 77.6290),
    "HSR Layout": (12.9116, 77.6424),
    "Agara": (12.9244, 77.6492),
    "Bellandur": (12.9259, 77.6765),
    "Marathahalli": (12.9594, 77.6967),
    "KR Puram": (13.0001, 77.7109),
    "Whitefield": (12.9696, 77.7494),
    "ITPL": (12.9783, 77.7486),
    "Kadugodi": (12.9896, 77.7569),
}
# Add default for others
for asset in all_assets:
    if asset not in station_coords:
        station_coords[asset] = (12.97, 77.59)  # default

# ---------------------------
# Data generation functions (simulate OT assets with Alstom Urbalis, HealthHub, etc.)
# ---------------------------
fake = Faker()
...
def generate_initial_assets():
    ...
    for asset in all_assets:
        ...
        if asset in depots:
            ...
            asset_type = "Phase 2 Station"

        # System type based on asset (to reflect Alstom Urbalis, etc.)
        if "Control Center" in asset_type or asset in control_centers:
            system_type = "Urbalis Vision Platform / SCADA Master"
        elif asset in phase2_stations or asset in purple_line_stations or asset in green_line_stations:
            system_type = random.choice(["Urbalis Zone Controller", "ATC Wayside", "PSD Controller", "Traction IED", "Station PLC"])
        elif asset in depots:
            system_type = random.choice(["Depot Management System", "Rolling Stock Maintenance", "HealthHub Gateway"])
        else:
            system_type = "OT Asset"
        ...
        plc_count = np.random.randint(5, 30) if "Station" in asset_type else np.random.randint(10, 50)
        rtu_count = np.random.randint(3, 20) if "Station" in asset_type else np.random.randint(5, 30)
...
def generate_initial_events(assets_df, num_events=150):
    ...
    event_types = [
        ...
        "Urbalis Zone Controller Anomaly",
        "HealthHub Data Tampering Attempt",
        "PSD Unauthorized Command"
    ]
    mitre_techniques = {
        ...
        "Urbalis Zone Controller Anomaly": "T0800",
        "HealthHub Data Tampering Attempt": "T0800",
        "PSD Unauthorized Command": "T0830"
    }
    events = []
...
asset_type_filter = st.sidebar.multiselect(
    "Asset Type", options=["Station", "Depot", "Control Center", "Phase 2 Station"],
    default=["Station", "Depot", "Control Center", "Phase 2 Station"]
)
line_filter = st.sidebar.multiselect(
    "Line", options=["Purple Line", "Green Line", "Phase 2 (Airport / Outer Ring Road)", "Depot", "Control Center"],
    default=["Purple Line", "Green Line", "Phase 2 (Airport / Outer Ring Road)", "Depot", "Control Center"]
)
zone_filter = st.sidebar.multiselect(
    "Zone", options=["East", "North", "South", "Central", "West", "Outer Ring Road Corridor", "Other"],
    default=["East", "North", "South", "Central", "West", "Outer Ring Road Corridor"]
)
...
with tab1:
    ...
    # Signaling & Train Control (Alstom Urbalis CBTC)
    st.subheader("🚆 Signaling & Train Control – Alstom Urbalis CBTC (GoA4 Driverless)")
    col1, col2 = st.columns(2)
    with col1:
        # Zone Controllers status (mock)
        zone_controllers = ["Zone Controller - Silk Board", "Zone Controller - KR Puram", "Zone Controller - Airport"]
        status = [random.choice(["Healthy", "Degraded", "Under Attack"]) for _ in zone_controllers]
        fig_zc = go.Figure(data=[go.Table(
            header=dict(values=["Zone Controller", "Status"], fill_color='paleturquoise', align='left'),
            cells=dict(values=[zone_controllers, status], fill_color='lavender', align='left')
        )])
        fig_zc.update_layout(title="Urbalis Zone Controllers")
        st.plotly_chart(fig_zc, use_container_width=True)

        # ATC command integrity
        atc_integrity = np.random.randint(85, 100)
        st.metric("ATC Command Integrity Score", f"{atc_integrity}%", delta="Stable")
    with col2:
        # Interlocking changes
        st.markdown('<div class="insight-text">🔒 Interlocking Logic Integrity</div>', unsafe_allow_html=True)
        st.write("Last interlocking table change: KR Puram junction – 2025-03-27 (authorized)")
        st.write("Pending changes: None")
        # Rogue device detection
        st.markdown('<div class="insight-text">🛡️ Rogue Devices on Signaling Network</div>', unsafe_allow_html=True)
        st.write("No unauthorized devices detected in last 24h")

    # Platform Screen Door (PSD) Security
    st.subheader("🚪 Platform Screen Door (PSD) Security")
    psd_stations = ["MG Road Interchange", "Silk Board", "KR Puram", "Airport Terminal"]
    psd_status = [random.choice(["OK", "Alert - Unauthorized command blocked", "Degraded"]) for _ in psd_stations]
    psd_df = pd.DataFrame({"Station": psd_stations, "PSD Status": psd_status})
    st.dataframe(psd_df, use_container_width=True)

    # Traction Power & Environmental Control
    st.subheader("⚡ Traction Power & Environmental Control")
    col1, col2 = st.columns(2)
    with col1:
        st.write("**Traction Substations Security**")
        substations = ["Peenya Depot", "Baiyappanahalli", "Kengeri", "KR Puram"]
        alerts = [random.choice(["Normal", "Failed login attempt", "Firmware mismatch"]) for _ in substations]
        st.dataframe(pd.DataFrame({"Substation": substations, "Status": alerts}), use_container_width=True)
    with col2:
        st.write("**Tunnel Ventilation PLCs**")
        st.write("All tunnel jet fan controllers reporting normal security posture.")
        st.write("Last anomaly: None")

    # Alstom HealthHub Integration
    st.subheader("🩺 Alstom HealthHub – Asset Health & Security Overlay")
    healthhub_assets = ["Train #3027 ATC", "Train #3012 Brake System", "Zone Controller Silk Board"]
    health_status = ["Degraded (unexpected firmware)", "Healthy", "Healthy"]
    security_status = ["Tampering suspected", "Normal", "Normal"]
    healthhub_df = pd.DataFrame({"Asset": healthhub_assets, "Operational Health": health_status, "Security Posture": security_status})
    st.dataframe(healthhub_df, use_container_width=True)
    if "Tampering" in healthhub_df["Security Posture"].values:
        st.warning("⚠️ HealthHub anomaly: Train #3027 ATC reporting unexpected firmware version. Verify with maintenance records.")

    # Safety Bowtie Prioritization (Example alerts)
    st.subheader("🚨 Safety Impact Alerts (Bowtie Prioritization)")
    st.markdown('<div class="alert-critical"><strong>Level 1 (Collision Risk):</strong> Interlocking modification detected – KR Puram junction without work order. Immediate investigation.</div>', unsafe_allow_html=True)
    st.markdown('<div class="alert-high"><strong>Level 2 (Service Disruption):</strong> Unauthorized command to traction breaker – Peenya substation. Potential service disruption if executed.</div>', unsafe_allow_html=True)
    st.markdown('<div class="alert-medium"><strong>Level 3 (Degraded Maintenance):</strong> HealthHub anomaly – Train #3027 ATC firmware mismatch. Investigate for potential tampering.</div>', unsafe_allow_html=True)
    ...
    fig_map = px.scatter_mapbox(
        ...
        hover_data={"type": True, "line": True, "zone": True, "vulnerability_count": True, "system_type": True},
...
with tab2:
    ...
    asset_display = filtered_assets[[
        "asset_name", "type", "line", "zone", "system_type", "health_score", "cyber_risk_score",
...
with tab4:
    ...
    station_assets = filtered_assets[filtered_assets["type"].isin(["Station", "Phase 2 Station"])]
...
# Tab 6: Data Ingestion & Architecture (Detailed – Based on BMRCL specifics)
with tab6:
    ...
    - **Stations (66 operational stations)** – Each station hosts:
    ...
    - **Phase 2 Corridors (Reach 6, 2A, 2B – 80.39 km, 50 stations)** – Include:
      - Alstom Urbalis CBTC (GoA4 Driverless) zone controllers, ATC wayside equipment, interlocking systems.
      - Platform screen doors (full-height at underground stations, half-height at grade).
    - **Depots (4)** – Peenya, Baiyappanahalli, Kengeri, and Phase 2 Depot (KR Puram):
    ...
    - **Control Centers (2)** – Main OCC (Baiyappanahalli) and Backup BCC (Peenya):
      - SCADA master servers, Alstom Urbalis Vision Platform, ATS (Automatic Train Supervision) servers, PSCADA (Power SCADA) systems.
    ...
    - **Level 1 (Control)**: Local controllers, PLC racks, RTU cabinets, Urbalis wayside units.
    - **Level 2 (Supervisory)**: SCADA servers, HMI workstations, engineering workstations, Urbalis Zone Controllers.
    - **Level 3 (Operations)**: Historian, asset management, network management, patch management, Alstom HealthHub.
    - **Level 4 (Enterprise)**: Corporate IT network (with strict DMZ separation).

    Communication protocols used: **Modbus/TCP, DNP3, IEC 60870-5-104, OPC UA, Profinet, Alstom Urbalis proprietary CBTC protocols**.
    ...
    |--------------------------------|-------------------------|------------------------------------------------|---------------------|
    | Alstom Urbalis CBTC            | Signaling, ATC commands | Syslog, SNMP traps, direct API                  | Real-time           |
    | SCADA Servers (PSCADA)         | Operational metrics     | OPC DA/UA, Modbus polling                       | 1 sec – 5 min       |
    ...
    | Alstom HealthHub               | Asset health, firmware  | REST API                                        | 5 min               |
    | CCTV System                    | Camera status, alerts   | API integration with existing SOC               | 1 min               |
    | Access Control System          | Badge logs              | API, syslog                                     | Real-time           |
    | NCIIPC Threat Intelligence     | Threat feeds            | API                                             | Hourly              |
    ...
    - **Threat Intelligence** feeds (MITRE ATT&CK for ICS, NCIIPC rail sector alerts).
    ...
    ### 5. Exhaustive List of OT Systems & Functions (Bangalore Metro Specific)
    | System/Asset                   | Function                                                                 |
    |--------------------------------|--------------------------------------------------------------------------|
    | **Alstom Urbalis CBTC**        | GoA4 driverless train control, zone controllers, ATC commands            |
    ...
    | **Alstom HealthHub**           | Predictive maintenance, real‑time condition monitoring of trains, infrastructure, signaling. |
    | **Platform Screen Door Controllers** | Secure operation of PSDs, integration with train arrival.           |
    """)

# Tab 7: Client Proposal (Slide Deck)
with tab7:
    ...
    - **Compliance** with IEC 62443, NIST CSF, NCIIPC guidelines.

    ### Why Deloitte?
    1. **Deep OT Expertise**: We have successfully deployed OT security programs for critical infrastructure clients globally, including rail and metro systems.
    ...
    - **Data Ingestion Layer**: Collect logs from Alstom Urbalis CBTC, SCADA, PLCs, network devices, and security appliances.
    - **SIEM Platform**: Centralized log management, correlation rules, dashboards (as demonstrated in this app).
    - **Network Monitoring**: Passive monitoring of OT protocols (Modbus, DNP3, Urbalis proprietary) to detect anomalies.
    - **Asset Management**: Automated discovery and vulnerability tracking.
    - **Threat Intelligence**: Tailored feeds for rail sector threats (NCIIPC, MITRE ATT&CK for ICS).
